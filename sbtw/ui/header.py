...
from sbtw.ui.projects import ProjectsGrid, ProjectsLabel


class Header(QWidget):
    def __init__(
        self, name: str, projects: list[dict] | None, parent: QWidget | None = None
    ):
        ...
        self.projects_grid = ProjectsGrid(projects=self.projects, parent=None)
        main_layout.addWidget(
            self.projects_label, alignment=Qt.AlignmentFlag.AlignRight
        )
        # ------------- Refresh -------------
    ...
    def toggle_projects_grid(self):
        if self.projects_grid.isVisible():
            self.projects_grid.hide()
        else:
            self.show_projects_grid()

    def show_projects_grid(self):
        # Position grid under the label
        global_bottom_right = self.projects_label.mapToGlobal(
            self.projects_label.rect().bottomRight()
        )

        x = global_bottom_right.x() - self.projects_grid.width()
        y = global_bottom_right.y()

        self.projects_grid.move(x, y)
        self.projects_grid.show()
